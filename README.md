...
- 更新 APN Topic 文档：`today_spark_unanswered` 推送的 `custom.data` 会携带序列化后的 `question_id`，客户端可用它直接打开对应 Today's Spark 问题页

### 2026-05-03

- 新增 `GET /api/weekly-report/prev` 接口：获取当前用户上周周报对应的 icon 列表，响应格式与 `GET /api/weekly-report/current` 保持一致，并额外返回 `period_start`、`period_end`、`read_at`

### 2026-04-29

- 新增 `GET /api/answers/:id`（Answer Detail）接口文档：返回单条 answer 的内容、题目快照、当前 question 分类信息及 icon 信息
- 更新 APN Topic 文档：`stamp_reveal` 推送的 `custom.data` 会携带序列化后的 `answer_id`，客户端可用它打开对应 answer detail / stamp reveal
...
- **URL**: `GET /api/answers/:id`
- **描述**: 获取当前用户某条回答的详情，可用于通过推送中的 `answer_id` 打开对应 Stamp Reveal / Answer Detail 页面
- **认证**: 需要
- **路径参数**:
  - `id`: 回答 ID
- **响应示例**:
  ```json
  {
    ...
    "data": {
      ...
      "question": {
        ...
        "title": "今天让你感到最温暖的小事是什么？",
        "category": {
          "id": "cludcategory123456789",
          "name": "生活感悟"
        },
        "sub_category": {
          "id": "cludsubcategory123456789",
          "name": "kindness"
        }
      },
      "icon": {
  ...
  ```
- **错误响应**:
  - 回答不存在、已删除或不属于当前用户：返回 `404 Not Found`
- **说明**:
  - `question_snapshot` 是用户作答时的问题文本快照；`question` 是当前问题及分类信息
  - `question.sub_category` 可能为 `null`
  - `icon` 字段说明同 `GET /api/answers` 接口；`error` 在生成失败时可能包含失败原因
...
- **描述**: 获取当前用户上周周报对应的 icon 列表。响应格式与 `GET /api/weekly-report/current` 保持一致，并额外返回上周周报周期和阅读状态。
- **认证**: 需要
- **响应示例**:
  ```json
  {
    ...
    "read_at": "2026-05-03T03:20:00.000Z",
    "icons": [
      {
        "id": "cludicon123456789",
        "answer_id": "cludanswer123456789",
        "created_ymd": "2026-04-21",
        "url": "https://your-oss-bucket.oss-region.aliyuncs.com/icons/cludicon123456789-1234567890.webp?Expires=1234567890&OSSAccessKeyId=xxx&Signature=xxx",
        "read_at": null
      },
      {
        "id": "cludicon987654321",
        "answer_id": "cludanswer987654321",
        "created_ymd": "2026-04-25",
        "url": "https://your-oss-bucket.oss-region.aliyuncs.com/icons/cludicon987654321-1234567890.webp?Expires=1234567890&OSSAccessKeyId=xxx&Signature=xxx",
        "read_at": "2026-04-26T12:00:00.000Z"
      }
    ]
  }
  ```
- **说明**:
  - `period_start` / `period_end`：上周周期的开始/结束日期（`YYYY-MM-DD`）
  - `read_at`：上周周报的阅读时间；`null` 表示未读或上周周报尚不存在
  - `icons` 来自上周周报记录中的 `icon_ids`
  - `icons[].url` 为签名后的可访问地址
  - `icons[].read_at`：对应 icon 的阅读时间；`null` 表示未读
  - 若上周周报不存在，返回上周周期、`read_at: null` 和空 `icons`
...
推送通知的 APN `payload.custom` 中包含 `topic` 字段，客户端根据该字段决定跳转行为。部分 topic 会额外携带 `custom.data`；该字段是 JSON 字符串，客户端需要先 `JSON.parse` 后使用。

| Topic                    | 触发场景                                    | `custom.data` 参数                         | 建议跳转目标                         |
| ------------------------ | ------------------------------------------- | ------------------------------------------ | ------------------------------------ |
| `stamp_reveal`           | Icon 生成成功（Stamp Ready）                | `{"answer_id":"cludanswer123456789"}`      | 对应 answer 的 Stamp Reveal / Detail |
| `stamp_thread`           | Icon LLM 判断为 Warn 或 Block               | 无                                         | Thread 界面                          |
| `weekly_report`          | Weekly Report 生成完成                      | 无                                         | Arcade 界面（Ready to Print）        |
| `today_spark_unanswered` | Daily Whisper，用户今日未回答 Today's Spark | `{"question_id":"cludquestion123456789"}`  | Today's Spark 对应问题页             |
| `daily_calendar`         | Daily Whisper，用户今日已回答 Today's Spark | 无                                         | Calendar 界面                        |

### `stamp_reveal` Payload 示例

```json
{
  "aps": {
    "alert": {
      "title": "Ta-da! A new stamp for you.",
      "body": "Your moment has taken shape. Come see the little mark you left today."
    }
  },
  "custom": {
    "topic": "stamp_reveal",
    "data": "{\"answer_id\":\"cludanswer123456789\"}"
  }
}
```

客户端收到 `stamp_reveal` 后，可解析 `custom.data.answer_id` 并调用 `GET /api/answers/:id` 获取对应回答详情。

### `today_spark_unanswered` Payload 示例
...
```json
{
  "aps": {
    "alert": {
      ...
      "body": "What felt bright today?"
    }
  },
  "custom": {
    "topic": "today_spark_unanswered",
    "data": "{\"question_id\":\"cludquestion123456789\"}"
  }
}
```

客户端收到 `today_spark_unanswered` 后，可解析 `custom.data.question_id` 并打开对应 Today's Spark 问题页。
